...
# event: 0: idle, 1: click, 2: press
def decideEvent(samples:list):
    eventDict = {'idle': 0, 'click': 1, 'press': 2}
    event = 0
    pos = samples[0].label
    if pos == -1:
        nonIdleCnt = 0
        for sp in samples[1:]:
            if sp.label != -1:
                nonIdleCnt += 1
        if nonIdleCnt > len(samples) * 0.25 and nonIdleCnt < len(samples) * 0.55:
            event = eventDict['click']
        elif nonIdleCnt >= len(samples) * 0.55:
            event = eventDict['press']
...
if __name__ == '__main__':
    # TODO: note: class num
    class_num = 2   # idle, click, press; why 2? : use class_num + 1
    allData = getData(TEST_POINT_NUM)
